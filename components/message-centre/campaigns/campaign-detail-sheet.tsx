  ...
  return (
    <Sheet open={Boolean(campaignId)} onOpenChange={(open) => !open && onClose()}>
      <SheetContent className="flex w-full flex-col gap-0 sm:max-w-2xl">
        {/* The title stays mounted while loading: Radix needs an accessible
            name on every Content, not only once the data arrives. */}
        <SheetHeader className="space-y-1 pb-3">
          <SheetTitle className="flex flex-wrap items-center gap-2">
            {data ? data.campaign.name : "Campaign"}
            {data ? (
          ...
          <SheetDescription>
            {data
              ? `${data.campaign.template_name || data.campaign.content_sid} · ${formatSgtDateTime(
